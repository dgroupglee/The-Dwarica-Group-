...
export default function AutomobilesRealm() {
  ...
  const submitInquiry = async (event) => {
    ...
    const item = { item_name: `${inquiry.vehicle.model} / ${inquiry.variation.label}`, item_category: 'Automobiles', message: String(data.get('message')), email: String(data.get('email')) };
    const { data: { user: currentUser } } = await supabase.auth.getUser();
    if (!currentUser || currentUser.is_anonymous) { window.localStorage.setItem('pendingInquiry', JSON.stringify(item)); setEmailPrompt({ type: 'inquiry', inquiry: item }); setInquiry(null); return; }
    const { error } = await supabase.from('inquiries').insert({ account_id: currentUser.id, ...item, status: 'received' });
  ...
  const submitConfiguration = async (event) => {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const item = { item_name: configureItem.vehicle.model, item_category: 'Automobiles configuration', item_reference: configureItem.variation.id, message: `Trim: ${data.get('trim')}\nColor: ${data.get('color')}\nMileage: ${data.get('mileage')}`, email: String(data.get('email')) };
    const { data: { user: currentUser } } = await supabase.auth.getUser();
    if (!currentUser || currentUser.is_anonymous) { window.localStorage.setItem('pendingInquiry', JSON.stringify(item)); setEmailPrompt({ type: 'inquiry', inquiry: item }); setConfigureItem(null); return; }
    const { error } = await supabase.from('inquiries').insert({ account_id: currentUser.id, ...item, status: 'received' });
    if (!error) { setConfigureItem(null); setSubmitted(true); }
  };
  ...
  return <section className="realm-shell">
    ...
    <div className="market-filters automobile-filters">{['All', 'Sedan', 'SUV'].map((item) => <button key={item} type="button" className={vehicleCategory === item ? 'market-filter market-filter--active' : 'market-filter'} onClick={() => setVehicleCategory(item)}>{item}</button>)}</div>
    ...
    {Object.entries(groupedVehicles).map(([make, makeVehicles]) => <div className="automobile-make-group" key={make}><div className="automobile-make-header"><h2>{make}</h2><span>{makeVehicles.length} configurations available</span></div><motion.div layout className="realm-grid realm-grid--three">{makeVehicles.map((vehicle) => { const variation = vehicle.variations.find((item) => item.id === selected[vehicle.id]); const isSaved = savedItems.includes(variation.id); return <motion.article layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="realm-card vehicle-card" key={vehicle.id}>
      <div className="inventory-image-placeholder"><span>{vehicle.model}</span></div><span className="inventory-label">In-hand configuration</span><h2>{vehicle.model}</h2><div className="vehicle-configs">{vehicle.variations.map((item) => <button type="button" key={item.id} className={variation.id === item.id ? 'config-pill config-pill--active' : 'config-pill'} onClick={() => setSelected((current) => ({ ...current, [vehicle.id]: item.id }))}>{item.label}</button>)}</div><div className="vehicle-basic-specs"><div><span>Year / variation</span><strong>{variation.label}</strong></div><div><span>Exterior</span><strong>{variation.color}</strong></div><div><span>Mileage</span><strong>{variation.mileage}</strong></div></div><strong className="inventory-price vehicle-price">{money(variation.price)}</strong><span className="ship-badge">Listed price / delivery on inquiry</span>
      <div className="inventory-actions"><button type="button" className={`favorite-button ${isSaved ? 'favorite-button--saved' : ''} ${favoritePulseId === variation.id ? 'favorite-button--pulse' : ''}`} aria-label={isSaved ? `Remove ${vehicle.model}` : `Save ${vehicle.model}`} aria-pressed={isSaved} onClick={() => toggleFavorite({ id: variation.id, name: `${vehicle.model} / ${variation.label}`, category: 'Automobiles', price: variation.price })}>♡</button><button type="button" className="inventory-hint-button" onClick={() => setDrawerItem({ ...variation, model: vehicle.model, detail: `${variation.color} exterior, ${variation.mileage}. Complete provenance record held in the private desk file.` })}>Open telemetry →</button><button type="button" className="secondary-button" onClick={() => setConfigureItem({ vehicle, variation })}>Configure your order</button><button type="button" className="primary-button" onClick={() => setInquiry({ vehicle, variation })}>Confirm interest</button></div>
      {isAccountHolder ? <div className="inventory-detail"><div><span>Trim specification</span><strong>{variation.label} full specification</strong></div><div><span>VIN / service file</span><strong>Verified in private desk</strong></div></div> : <div className="inventory-private-note" role="button" tabIndex="0" onClick={() => setEmailPrompt({ type: 'dossier_unlock', assetId: variation.id })}><span>PRIVATE INTELLIGENCE — ACCOUNT HOLDERS ONLY</span><em>Create your private DGroup account to access full asset intelligence.</em></div>}
    </motion.article>; })}</motion.div></div>)}
    <TelemetryDrawer item={drawerItem} onClose={() => setDrawerItem(null)} category="Automobile" />
    {inquiry ? <div className="market-modal-backdrop" onClick={() => setInquiry(null)}><div className="market-modal" onClick={(event) => event.stopPropagation()}><button type="button" className="modal-close" onClick={() => setInquiry(null)} aria-label="Close inquiry">×</button><span className="section-kicker">Private inquiry</span><h2>{inquiry.vehicle.model}</h2><p>{inquiry.variation.label} · {money(inquiry.variation.price)}</p><form onSubmit={submitInquiry}><label>Your Email<input required type="email" name="email" placeholder="your@email.com" /></label><label>Message<textarea required rows="4" name="message" placeholder="Specification, history, delivery." /></label><button type="submit" className="primary-button">Submit Inquiry</button></form></div></div> : null}
    {configureItem ? <div className="market-modal-backdrop" onClick={() => setConfigureItem(null)}><div className="market-modal configure-order-modal" onClick={(event) => event.stopPropagation()}><button type="button" className="modal-close" onClick={() => setConfigureItem(null)} aria-label="Close configuration">×</button><span className="section-kicker">Automotive configuration desk</span><h2>Configure your order.</h2><p>{configureItem.vehicle.model} / {configureItem.variation.label}</p><form onSubmit={submitConfiguration}><label>Trim preference<input required name="trim" defaultValue={configureItem.variation.label} /></label><label>Color preference<input required name="color" defaultValue={configureItem.variation.color} /></label><label>Mileage preference<input required name="mileage" defaultValue={configureItem.variation.mileage} /></label><label>Direct email<input required type="email" name="email" placeholder="your@email.com" /></label><button type="submit" className="primary-button">Route configuration request</button></form></div></div> : null}
