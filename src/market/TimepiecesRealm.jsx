      ...
      <motion.div layout className="realm-grid realm-grid--three"><AnimatePresence mode="popLayout">
        {visibleItems.map((item) => {
          ...
          return <motion.article layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: .94 }} transition={{ duration: .45 }} data-velocity key={item.id} className={`realm-card inventory-card ${isOpen ? 'inventory-card--expanded' : ''}`}>
              ...
              <span className="inventory-label">{item.brand}</span><h2>{item.model}</h2><p>{item.descriptor}</p><strong className="inventory-price">{formatPrice(item.price)}</strong><span className={`inventory-status ${statusClass(item.status)}`}>{item.status || 'Available'}</span><span className="ship-badge">{item.ship}</span><div className="inventory-public-specs" aria-label={`${item.model} public specifications`}><div><span>Production year</span><strong>{item.year || '2021'}</strong></div><div><span>Case size</span><strong>{item.case_size || 'Reference-specific'}</strong></div><div><span>Dial / finish</span><strong>{item.dial_color || item.descriptor}</strong></div><div><span>Box & papers</span><strong>{item.papers}</strong></div></div>
            </button>
