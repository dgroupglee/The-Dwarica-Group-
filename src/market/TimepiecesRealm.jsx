  ...
  return (
    <section className="realm-shell">
      ...
      <div className="market-filter-bar">
        <div className="market-filters">{['All', 'Watches', 'Chains & Links', 'Bracelets', 'Rings'].map((item) => <button key={item} type="button" className={category === item ? 'market-filter market-filter--active' : 'market-filter'} onClick={() => setCategory(item)}>{item}</button>)}</div>
      ...
      <motion.div layout className="realm-grid realm-grid--three"><AnimatePresence mode="popLayout">
        {visibleItems.map((item) => {
          ...
          return <motion.article layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: .94 }} transition={{ duration: .45 }} data-velocity key={item.id} className={`realm-card inventory-card ${isOpen ? 'inventory-card--expanded' : ''}`}>
              ...
              <span className="inventory-label">{item.brand}</span><h2>{item.model}</h2><p>{item.descriptor}</p><strong className="inventory-price">{item.price ? formatPrice(item.price) : 'Inquire for pricing'}</strong><span className="ship-badge">{item.ship}</span>
            </button>
