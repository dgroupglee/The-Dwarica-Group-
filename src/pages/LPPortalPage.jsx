...
const fallbackInventory = [
  { id: 'lp-royal-oak', brand: 'Audemars Piguet', model: 'Royal Oak Chronograph 26240OR', price: 118000, category: 'timepieces' },
  { id: 'lp-rm-055', brand: 'Richard Mille', model: 'RM 055', price: 320000, category: 'timepieces' },
  { id: 'lp-cuban', brand: 'DGroup Private Collection', model: '18K White Gold Cuban Link', price: 28500, category: 'timepieces' },
];
...
export default function LPPortalPage() {
  ...
  const [activeInventory, setActiveInventory] = useState(fallbackInventory);
  ...
  useEffect(() => {
    if (!verifiedLp) return undefined;
    let active = true;
    supabase.from('listings').select('id, brand, model, price, category').eq('available', true).order('price', { ascending: false }).limit(3).then(({ data }) => { if (active && data?.length) setActiveInventory(data); }).catch(() => undefined);
    return () => { active = false; };
  }, [verifiedLp]);
